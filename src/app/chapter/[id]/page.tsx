...
useEffect(() => {
  ...
  return (
    <>
      ...
      <div className="h-[calc(100vh-80px)] w-full md:max-w-7xl md:mx-auto flex flex-col overflow-hidden">
        <motion.div
        ...
        <motion.div
          ...
          className="flex justify-between px-2 sm:px-6 py-2 border-t border-gray-800 bg-gray-900"
        >
          <button
            disabled={!neighbors.previous}
            onClick={() => neighbors.previous && router.push(`/chapter/${neighbors.previous._id}`)}
            className={`px-1 sm:px-4 py-1 rounded-xl text-sm ${
              neighbors.previous
          ...
          <button
            ...
            className={`px-1 sm:px-4 py-3 rounded-xl text-sm ${
              neighbors.next
                ? 'cursor-pointer bg-gradient-to-r from-gray-800 to-gray-900 hover:from-gray-700 hover:to-gray-800'
                : 'bg-gray-700 opacity-50 cursor-not-allowed'
            } transition text-gray-300 shadow-lg`}
