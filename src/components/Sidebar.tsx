...
export default function Sidebar({ open, onClose }: { open: boolean, onClose: () => void }) {
  ...
  return (
    <>
      ...
      {open && (
        <div 
          className="fixed inset-0 top-16 bg-black opacity-5 bg-opacity-50 z-40 md:hidden"
          onClick={onClose}
        />
      )}
      ...
      <aside
        className={`
          fixed inset-0 top-16 bg-gray-900 text-white z-50
      ...
      >
        {/* Close button mobile */}
        <div className="flex items-center justify-start p-4 lg:hidden">
          <button onClick={onClose} className="text-white hover:text-yellow-400">
            <ArrowLeft size={24} />
          </button>
        </div>
